...
Or let `bank_calibrate.py` do the whole loop — build, render a
self-generated calibration MIDI through both fluidsynth (ground truth)
and the offline device model, compare per-program levels, and rebuild
with corrective trims until every program and drum key sits within a
tolerance (default ±1 dB) of the source font's own balance:

```sh
python3 bank_calibrate.py /path/to/your_font.sf2 \
    --out gmbank.bin --trims-out trims_cal.json --report calib_report.txt
```

`calib_report.txt` is the equivalence proof (before/after residual per
program); `trims_cal.json` is the reproducible recipe; requires
`fluidsynth` on PATH. No MIDI files are needed — the calibration score
is generated by the tool. Works with any font, so the balance always
matches *your* chosen reference.
